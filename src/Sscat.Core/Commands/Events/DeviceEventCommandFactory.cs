        ...
        public override IEventCommand CreateCommand()
        {
            switch (_item.Id)
            {
                ...
                case Constants.DeviceType.PLA_MATCHING_ITEM_RESPONSE:
                    return new PLAMatchingItemResponse(Hooks[ScotLogHook.Traces], _item, Lane, Timeout, EnableHook);
                case Constants.DeviceType.PLA_VERIFY_ITEM_INTERVENTION:
                    return new PLAVerifyItemIntervention(Hooks[ScotLogHook.Traces], _item, Lane, Timeout, EnableHook);
                case Constants.DeviceType.PLA_SHOW_MESSAGE:
                    return new PLAShowMessage(Hooks[ScotLogHook.Traces], _item, Lane, Timeout, EnableHook);
#if NET40
                ...
                case Constants.DeviceType.ESA_EMULATOR:
                    return new ESAIntervention(Hooks[ScotLogHook.Traces], _item, Lane, Timeout, EnableHook);
#endif
                default:
