    ...
    public class LaunchApplication : ApplicationLauncherEventCommand
    {
        ...
        public override void Run()
        {
            try
            {
                ...
                Result = new Result(ResultType.Passed, string.Format(@"EventDetails = Event[ApplicationLauncher] : {0}", Item.ApplicationPath));
            }
            catch (IndexOutOfRangeException exc)
            {
                string notes = string.Format("; ErrorMessage = {1} {2}Troubleshooting tips: {2}Correct the value in your script Path = \"{0}\", the pattern is not yet supported.", Item.ApplicationPath, exc.Message, Environment.NewLine);
                Result = new Result(ResultType.Failed, notes);
                LoggingService.Info(notes);
            }
            catch (Exception ex)
            {
                LoggingService.Info(ex.ToString());
                Result = new Result(ResultType.Warning, Regex.Replace(ex.Message, @"\t|\n|\r", "; "));
            }
            finally
            {
                ApplicationLauncher.ApplicationLauncherManage -= new EventHandler<ApplicationLauncherEventArgs>(ApplicationLauncherManage);
