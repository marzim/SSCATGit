            ...
            try
            {
                ...
                if (!lane.HasErrors)
                {
                    ...
                    if (!lane.HasStopped)
                    {
                        ...
                        OnDispatching(string.Format("{0}Script(s) Generated.{1}Path :{2}{1}Script generation done.", _generatorConfiguration.Scripts.Scripts.Length, Environment.NewLine, _generatorConfiguration.ScotConfigOutputDirectory));
                        if (_generatorConfiguration.Scripts.Scripts.Length == 0)
                        { 
                            OnDispatching(string.Format("{0}No Scripts were generated. No complete valid transaction was found in SSCO logs.{0}Troubleshooting Tips : {0}Kindly perform a complete and valid transaction manually before generating a script.", Environment.NewLine));
                        }
