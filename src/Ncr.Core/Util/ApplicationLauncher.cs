...
namespace Ncr.Core.Util
{
    ...
    public class ApplicationLauncher : IApplicationLauncher
    {
        ...
        public bool RunApplication(string host, string path)
        {
            ...
            string username = FileHelper.GetIniValue("ApplicationLauncherLoginCredentials", "Username", ApplicationUtility.SettingsFileName);
            string password = FileHelper.GetIniValue("ApplicationLauncherLoginCredentials", "Password", ApplicationUtility.SettingsFileName);
            string[] args = path.Split(' ');

            if (CanRunOnRemoteHost(host, args[0]))
            {
                ...
                else
                {
                    ...
                    p.StartInfo.Arguments = args.Length > 1 ? args[1] : string.Empty;
                    success = p.Start();
                }
            }
