// <copyright file="RegistryManager.cs" company="NCR">
//     Copyright 2017 NCR Corporation. All rights reserved.
// </copyright>
namespace Ncr.Core.Util
{
    using System.Collections.Generic;
    using Microsoft.Win32;
    
    /// <summary>
    /// Initializes a new instance of the RegistryManager class
    /// </summary>
    public class RegistryManager : IRegistryManager
    {
        /// <summary>
        /// Checks if registry key exists
        /// </summary>
        /// <param name="key">key name</param>
        /// <param name="subKey">registry sub key</param>
        /// <returns>Returns true if exists, false otherwise</returns>
        public bool Exists(string key, RegistryKey subKey)
        {
            return subKey.OpenSubKey(key) != null;
        }

        /// <summary>
        /// Get value from registry
        /// </summary>
        /// <param name="name">key name</param>
        /// <param name="subKey">registry sub key</param>
        /// <returns>Returns registry value</returns>
        public string GetValue(string name, RegistryKey subKey)
        {
            return (string)subKey.GetValue(name);
        }

        /// <summary>
        /// Get value from registry
        /// </summary>
        /// <param name="name">key name</param>
        /// <param name="subKey">registry sub key</param>
        /// <param name="defaultValue">default value</param>
        /// <returns>Returns registry value</returns>
        public string GetValue(string name, RegistryKey subKey, string defaultValue)
        {
            if (subKey != null)
            {
                string val = (string)subKey.GetValue(name);
                if (string.IsNullOrEmpty(val))
                {
                    return defaultValue;
                }

                return val;
            }

            return defaultValue;
        }

        /// <summary>
        /// Get value from registry
        /// </summary>
        /// <param name="key">key name</param>
        /// <param name="subKey">registry sub key</param>
        /// <param name="defaultValue">default value</param>
        /// <param name="disregardComments">whether or not to disregard comments</param>
        /// <returns>Returns registry value</returns>
        public string GetValue(string key, RegistryKey subKey, string defaultValue, bool disregardComments)
        {
            string val = GetValue(key, subKey, defaultValue);
            if (disregardComments)
            {
                string[] valOnly = val.Split(' ');
                return valOnly[0];
            }
            else
            {
                return val;
            }
        }

        /// <summary>
        /// Set string value
        /// </summary>
        /// <param name="keyName">key name</param>
        /// <param name="keyValueName">key value name</param>
        /// <param name="keyValueData">key value data</param>
        public void SetStringValue(string keyName, string keyValueName, string keyValueData)
        {
            Registry.SetValue(keyName, keyValueName, keyValueData, RegistryValueKind.String);
        }

        /// <summary>
        /// Set string value
        /// </summary>
        /// <param name="keyName">key name</param>
        /// <param name="keyValueName">key value name</param>
        /// <param name="keyValueData">key value data</param>
        /// <param name="overwrite">whether or not to overwrite previous value</param>
        public void SetStringValue(string keyName, string keyValueName, string keyValueData, bool overwrite)
        {
            RegistryKey key = Registry.LocalMachine.OpenSubKey(keyName, true);
            try
            {
                if ((key.GetValue(keyValueName) != null) && !overwrite)
                {
                    return;
                }

                key.SetValue(keyValueName, keyValueData, RegistryValueKind.String);
            }
            catch (System.ArgumentNullException)
            {
            }
            catch
            {
                throw;
            }
        }

        /// <summary>
        /// Delete the sub key
        /// </summary>
        /// <param name="subKey">sub key</param>
        public void DeleteSubKey(string subKey)
        {
            Registry.LocalMachine.DeleteSubKey(subKey);
        }

        /// <summary>
        /// Get the sub key names
        /// </summary>
        /// <param name="key">key name</param>
        /// <returns>Returns the sub key names</returns>
        public string[] GetSubKeyNames(string key)
        {
            string[] subKeyNames = null;
            if (Exists(key, Registry.LocalMachine))
            {
                subKeyNames = Registry.LocalMachine.OpenSubKey(key).GetSubKeyNames();
            }

            return subKeyNames;
        }

        /// <summary>
        /// Get the sub key names
        /// </summary>
        /// <param name="key">key name</param>
        /// <param name="subKeyNameContains">sub key name contains</param>
        /// <returns>Returns the sub key names</returns>
        public List<string> GetSubKeyNames(string key, string subKeyNameContains)
        {
            string[] tempSubKeyNames = GetSubKeyNames(key);
            List<string> subKeyNames = new List<string>();
            foreach (string s in tempSubKeyNames)
            {
                if (s.Contains(subKeyNameContains))
                {
                    subKeyNames.Add(s);
                }
            }

            return subKeyNames;
        }

        /// <summary>
        /// Open sub key
        /// </summary>
        /// <param name="key">registry key</param>
        /// <param name="name">key name</param>
        /// <returns>Returns the registry key</returns>
        public virtual RegistryKey OpenSubKey(RegistryKey key, string name)
        {
            return key.OpenSubKey(name);
        }
    }
}
