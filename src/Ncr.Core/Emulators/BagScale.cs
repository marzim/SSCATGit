...
namespace Ncr.Core.Emulators
{
    ...
    public class BagScale : Scale, IBagScale
    {
        ...
        {
            string BagUnit = BagScaleUnit.Trim() == "1" ? "Grams" : BagScaleUnit.Trim() == "2" ? "Kilograms" : BagScaleUnit.Trim() == "3" ? "Ounce" : BagScaleUnit.Trim() == "4" ? "Pounds" : "Unidentified scale unit";
            LoggingService.Info(string.Format("Detected bag scale unit: {0}", BagUnit));
