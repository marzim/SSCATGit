...
namespace Ncr.Core.Emulators
{
    ...
    public class Scale : Emulator, IScale, ITear
    {
        ...
        /// </summary>
        /// <param name="timeout">timeout amount</param>
        /// <returns>the scale weight</returns>
        public virtual int GetScaleWeight(int timeout)
        {
            string scaleText = GetText("ProduceScale_ScaleWeight", timeout);
            int scaleWeight = 0;
            try
            {
                scaleWeight = Int32.Parse(scaleText);
            }
            catch (Exception ex)
            {
