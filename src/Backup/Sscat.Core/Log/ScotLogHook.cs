...
namespace Sscat.Core.Log
{
    ...
    public class ScotLogHook : LogHook, IScotLogHook
    {
        ...
        public virtual Result Check(IScriptEvent scriptEvent, int timeout)
        {
            ...
            string notfound = string.Format("No log was found in {0}. Event: {1}", _parser.Name, scriptEvent.ToRepresentation());
            try
            {
                ...
                {
                    IWarningEvent evnt = new WarningEvent("Log Mismatch", string.Format("Expected: {0}; Actual: {1}", scriptEvent.Item.ToString(), lastSimilarEvent.Item.ToString()));
                ...
                {
                    IWarningEvent evnt = new WarningEvent("Log Not Found", string.Format("Expected: {0}", scriptEvent.Item.ToString()));
                    _warningevnts.Add(evnt);
                    OnWarningFound(new WarningEventArgs(evnt));
            ...
            {
                IWarningEvent evnt = new WarningEvent("ReceiptItemException", string.Format("ScriptEvent: {0}, {1}", scriptEvent.Item.ToString(), e.Message));
                _warningevnts.Add(evnt);
                OnWarningFound(new WarningEventArgs(evnt));
        ...
        public virtual bool Exists(IScriptEvent scriptEvent, out IScriptEvent lastSimilarEvent, int timeout)
        {
            ...
            while ((Environment.TickCount - now) < timeout && !found)
            {
                ...
                {
                    OnChecking(new SscatEventArgs(string.Format("Checking {0} Log...", _parser.Name)));
#if NET40                    
