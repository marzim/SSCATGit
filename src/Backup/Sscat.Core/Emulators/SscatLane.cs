...
namespace Sscat.Core.Emulators
{
    ...
    public class SscatLane : Lane
    {
            ...
            if (!HasErrors)
            {
                ...
                try
                {
                    ...
                    if (!HasStopped)
                    {
                        ...
                        List<IScript> s = SSCATScript.CreateScripts(name, description, scriptsOutputDirectory, sscoBuild, build, defaultMSCard, events, segmented, generateLast, lastScriptsNumber, Configuration.GeneratorConfiguration.UIValidation);
        ...
        public Report Play(IScript script, int timeout, int warningTimeout, bool enableHook, int repeat)
        {
            ...
            OnLoggerStart(null);
            OnPlaying("-----------------------------------------------------");
            OnPlaying(string.Format("Playing '{0}' with Index {1}; Repetition={2} ", script.FileName, script.Index, repeat));
            ...
            try
            {
                ...
                if (!IsScriptStartLogin(script.Events))
                {
                ...
                script.Events.AddEvents(GetWarningList());
        ...
        public void ValidateScriptname(ref string scriptname, string scriptsOutputDirectory)
        {
            ...
            AddErrorIf("You may have used the same script name before and forgot to delete the corresponding \"Configuration Folder\" where SSCAT saves all the important SSCO config files. Please delete/rename the folder or choose another script filename.", Directory.Exists(scriptsOutputDirectory + "\\" + scriptname));
        }
        ...
        private void NXTUIGoBack(string context)
        {
            ...
            {
                ThreadHelper.Sleep(200);
                OnEmulating(string.Format("[SmartExit] SSCAT encountered an error: '{0}' Context not defined in NXTUIBackCommands in LaneConfiguration.xml", context));
            }
        }
        ...
        {
            OnEmulating(string.Format("Going back from {0}...", context));
        ...
        /// </summary>
        /// <param name="events">scripts events</param>
        /// <returns>Returns true if script starts in Login</returns>
        private bool IsScriptStartLogin(ScriptEvents events)
        {
            for (int i = 0; i < 3; i++)
            {
                if (events.Events[i].Item is IUIAutoTestEvent)
                {
                    UIAutoTestEvent startEvent = events.Events[i].Item as UIAutoTestEvent;
                    if (startEvent.ContextName.Equals("Closed") && startEvent.ButtonName.Equals("StoreLogin") && NextGenUITestClient.Instance.GetCurrentContext().Equals("Closed"))
                    {
                        return true;
                    }
                }
