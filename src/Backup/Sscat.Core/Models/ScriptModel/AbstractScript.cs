...
namespace Sscat.Core.Models.ScriptModel
{
    ...
    public abstract class AbstractScript<T> : BaseModel<T>, IScript
    {
        ...
        public static List<IScript> CreateScripts(string name, string description, string path, string sscoBuild, string build, string defaultMSCard, List<IScriptEvent> events, bool segmented, bool generateLast, int lastScriptsNumber, bool uiValidation)
        {
#if NET40
        ...
        public static List<IScriptEvent> RemoveUIValidation(List<IScriptEvent> events)
        {
            ...
            try
            {
                while (i < max)
                {
                    ...
                    {
                        LoggingService.Info(string.Format("Removing {0}", events[i].ToRepresentation()));
                    }
