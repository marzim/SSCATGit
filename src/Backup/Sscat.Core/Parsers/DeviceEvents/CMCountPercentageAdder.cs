        ...
        public override void Add(string line, Match match, IExtendedTextReader reader, List<IScriptEvent> events, string eventValue)
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if ((m = Regex.Match(line, @".*DM-DMp.* \+GetPrinterPaperLow.*", RegexOptions.IgnoreCase)).Success)
                {
                    ...
                    events.Add(itemCount.CreateEvent(ConvertUtility.ToInt64(StringUtility.Replace(match.Groups["ms"].Value, ",", string.Empty)), true));
                    ...
                    events.Add(itemValue.CreateEvent(ConvertUtility.ToInt64(StringUtility.Replace(match.Groups["ms"].Value, ",", string.Empty)), true));
                    break;
                }
            }
