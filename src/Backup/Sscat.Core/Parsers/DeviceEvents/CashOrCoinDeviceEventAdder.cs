...
namespace Sscat.Core.Parsers.DeviceEvents
{
    ...
    public class CashOrCoinDeviceEventAdder : DeviceEventAdder
    {
        ...
        public override void Add(string line, Match match, IExtendedTextReader reader, List<IScriptEvent> events, string eventValue)
        {
            ...
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) .* inserted");
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](.)?(?<value>\d+)\&comma\;(?<value2>\d+) .* inserted");
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) .* įdėta");
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) .* hineingesteckt");
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) Kč vložen"); // TescoCZ
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) € insertado"); // ECI PDS2 Castilian (Spain) 0C0A EURO
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\]Włożono (?<value>\d+)\&comma\;(?<value2>\d+) zł"); // Poland
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) EUR behelyezve"); // TescoHU EUR Configuration
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) Ft behelyezve"); // TescoHU HUF Base - [SummaryX]200&comma;00 Ft behelyezve
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\&comma\;(?<value2>\d+) € Inséré"); // Monoprix French EUR 040C - [SummaryX]5&comma;00 € Inséré
            patterns.Add(@"(?<ms>\d{10}) .*\[SummaryX\]已插入HK\$(?<value>\d+).(?<value2>\d+)"); // ASW-Global Parknshop HK$ [SummaryX]已插入HK$10.00 or [SummaryX]已插入HK$100.00
            ...
            patterns2.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+)\s(?<value2>\d+) Ft behelyezve"); // TescoHU Hungarian Forint HUF
            patterns2.Add(@"(?<ms>\d{10}) .*\[SummaryX\]kr (?<value>\d{1}.\d{3})\&comma\;(?<value2>\d+) indsat"); // Danish Krone DKK - [SummaryX]kr 1.000&comma;00 indsat
            patterns2.Add(@"(?<ms>\d{10}) .*\[SummaryX\]kr (?<value>\d+)\&comma\;(?<value2>\d+) indsat"); // Danish Krone DKK - Match other denomination

            List<string> patterns3 = new List<string>(); // For cash/coin pattern that has only one value, example: [SummaryX]500 Ft behelyezve
            patterns3.Add(@"(?<ms>\d{10}) .*\[SummaryX\](?<value>\d+) Ft behelyezve"); // TescoHU Hungarian Forint HUF single value
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if ((m = Regex.Match(line, @"(?<ms>\d{10}) .*\[SummaryX\](.)?(?<value>\d+.\d+) (inserted|insertado)", RegexOptions.IgnoreCase)).Success)
                {
                    IDeviceEvent item = new DeviceEvent();
                    item.Id = Id;
