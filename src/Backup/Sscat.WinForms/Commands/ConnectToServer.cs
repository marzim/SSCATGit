            ...
            try
            {
                ...
                controller.ScriptFileNameCheck += delegate(object sender, GeneratorConfigurationEventArgs e)
                {
                    try
                    {
                        ...
                        e.Config.AddErrorIf("You may have used the same script name before and forgot to delete the corresponding \"Configuration Folder\" where SSCAT saves all the important SSCO config files. Please delete/rename the folder or choose another script filename.", Directory.Exists(e.Config.ScriptOutputDirectory + "\\" + e.Config.ScriptName));
                    }
                    catch (Exception ex)
                    {
